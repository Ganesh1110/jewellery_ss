...
export function HeroSlider({
  ...
  return (
    <section className={cn('relative flex items-center overflow-hidden', className)}>
      ...
      {/* Content */}
      <div className="container relative z-10 py-24 sm:py-32 lg:py-40">{children}</div>

      {/* Navigation */}
      <div className="absolute inset-y-0 left-0 right-0 flex items-center justify-between px-4 sm:px-6 z-10 pointer-events-none">
        <button
          type="button"
          onClick={() => goTo(current - 1)}
          aria-label="Previous slide"
          className="pointer-events-auto w-11 h-11 rounded-full bg-cream-50/10 text-cream-50 backdrop-blur flex items-center justify-center transition-colors hover:bg-gold-500 hover:text-neutral-950"
        >
