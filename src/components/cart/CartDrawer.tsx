...
import { X, Plus, Minus, Trash2, Gift, Truck, RotateCcw, ShoppingBag } from 'lucide-react';
...
export function CartDrawer() {
  ...
  return (
    <>
      ...
      />
      <aside
        className="fixed top-0 right-0 bottom-0 z-50 w-full max-w-md bg-white shadow-strong animate-slide-in flex flex-col"
        role="dialog"
        aria-modal="true"
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          {isEmpty ? (
            <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-center">
              <ShoppingBag className="h-16 w-16 text-neutral-300 mb-4" aria-hidden="true" />
              ...
              <p className="text-body text-neutral-500 mb-6 max-w-xs">
                Looks like you haven&apos;t added any pieces yet. Explore our collections to find something special.
              </p>
            ...
            <>
              ...
              <div className="mt-8 pt-6 border-t border-neutral-200">
                <label htmlFor="cart-note" className="flex items-center gap-2 text-body-sm font-medium text-neutral-700 mb-3">
                  <Gift className="h-4 w-4 text-gold-600" aria-hidden="true" />
                  Gift note (optional)
                </label>
                <textarea
                  id="cart-note"
                ...
                <p id="note-help" className="mt-1.5 text-caption text-neutral-500">
                  We&apos;ll include your note on a complimentary card with the order.
                </p>
              </div>
...
function CartItem({ line, currencyCode, onQuantityChange, onRemove }: CartItemProps) {
  ...
  const { title, selectedOptions, image, price, compareAtPrice, sku } = merchandise;
  const lineTotal = cost.totalAmount.amount;
  const unitPrice = cost.amountPerQuantity.amount;

  const hasGiftWrap = attributes?.some((attr) => attr.key === 'Gift Wrap');

  return (
    <li className="flex gap-4 animate-slide-up">
      <div className="relative w-20 h-20 flex-shrink-0 overflow-hidden bg-neutral-50">
        <OptimizedImage
          src={image?.url}
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between">
        <div>
          ...
          {hasGiftWrap && (
            <span className="mt-1.5 inline-flex items-center gap-1 text-caption text-gold-600">
  ...
  return (
    <div className="flex items-center border border-neutral-300 rounded">
      <button
        ...
        disabled={quantity <= 1}
        className="p-2 text-neutral-700 hover:text-neutral-950 hover:bg-neutral-100 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
      ...
      <button
        onClick={() => onChange(line, 1)}
        className="p-2 text-neutral-700 hover:text-neutral-950 hover:bg-neutral-100 transition-colors"
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-body-sm font-medium text-neutral-700">
            {progress >= 100 ? 'Complimentary shipping unlocked!' : 'Add more for complimentary shipping'}
          </span>
        </div>
        {progress < 100 && (
          <span className="text-body-sm font-medium text-gold-600 tabular-nums">
            {formatMoney(remaining, currencyCode)} remaining
          </span>
        )}
      </div>
      <div className="h-1 bg-neutral-200 rounded-full overflow-hidden">
        <div
          className="h-full bg-gold-500 transition-all duration-500 ease-out-expo"
